...
//نمایش مدیا دریافتی از دوربین به کاربر در صفحه
...
// غیر فعال سازی دوربین
function stopCamera() {
    console.log('Stop User Camera');
...
function startRecording() {
    ...
    try {
        //ارسال رشته دریافتی جهت ضبط
        if (remoteStream != null) {
            console.log('Record Remote Stream...');
            mediaRecorder = new MediaRecorder(remoteStream, options);
        }
        else
        {
            console.log('Remote Stream Is Null!!');
            console.log('Record Local Stream...');
...
//دریافت تصویر در بدو ورود به صفحه
grabWebCamVideo();
...
$('#roomTable tbody').on('click', 'button', function () {
    if (hasRoomJoined) {
        ...
        console.log('Join on Room: ' + myRoomId);
    }
});
